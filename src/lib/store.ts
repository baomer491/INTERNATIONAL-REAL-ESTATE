...
/* ===== Password Hashing (PBKDF2-SHA256, compatible with schema seed) ===== */
...
function reportToSnake(report: Partial<Report>): Record<string, unknown> {
  ...
  if (report.bankId !== undefined) m.bank_id = report.bankId;
  if (report.bankName !== undefined) m.bank_name = report.bankName;
  if (report.beneficiaryId !== undefined) m.beneficiary_id = report.beneficiaryId;
  ...
  if (report.appraiserId !== undefined) m.appraiser_id = report.appraiserId;
  ...
  updateReport: (id: string, data: Partial<Report>): void => {
    ...
    if (idx !== -1) {
      ...
      db.from('reports').update(row).eq('id', id).then(({ error }) => {
        if (error) console.error('[store] updateReport error:', error.message);
      });
    }
  },
