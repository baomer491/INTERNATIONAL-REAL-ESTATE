...
export interface Task {
  ...
  status: TaskStatus;
  originalStatus?: TaskStatus; // status before becoming overdue
  dueDate: string;
  createdAt: string;
  updatedAt?: string;
  ...
  timeSpent?: number; // minutes spent in in_progress
  startedAt?: string; // when moved to in_progress
  reviewedAt?: string; // when moved to under_review
  comments?: { id: string; text: string; createdAt: string; createdBy?: string; createdByName?: string }[];
...
export const CATEGORY_DUE_DAYS: Record<TaskCategory, number> = {
  general: 3,
  valuation: 5,
  followup: 2,
  administrative: 3,
